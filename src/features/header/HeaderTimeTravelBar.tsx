...
export const HeaderTimeTravelBar: React.FC<HeaderTimeTravelBarProps> = ({
  ...
  const theme = isGlobal
    ...
    : {
        badgeBg: isActive ? 'bg-sky-100/70' : 'bg-slate-100',
        badgeText: isActive ? 'text-sky-900' : 'text-slate-700',
        metricBg: isActive ? 'bg-white/60' : 'bg-white/60',
        metricText: isActive ? 'text-sky-800' : 'text-slate-600',
        secondaryText: isActive ? 'text-sky-700' : 'text-slate-500',
        buttonBg: 'bg-sky-600 hover:bg-sky-700',
        icon: Clock,
        iconColor: isActive ? 'text-sky-600' : 'text-slate-400',
        scopeLabel: 'EVENTI',
        liveLabel: 'LIVE',
