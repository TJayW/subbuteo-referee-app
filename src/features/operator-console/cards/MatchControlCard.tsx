  ...
  return (
    <div className="ui-surface p-4 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <ActionButton action={isPlaying ? 'warning' : 'success'} size="sm" onClick={onPlayPause} className="flex-1">
            {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
            {isPlaying ? 'Pausa' : 'Avvia'}
          </ActionButton>
          <ActionButton action="primary" size="sm" onClick={onEndPeriod} className="flex-1">
            <Flag className="w-4 h-4" />
            Termina periodo
          </ActionButton>
        </div>
        <div className="flex items-center gap-1">
          {timeAdjustments.map((delta) => (
            <button
              key={delta}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs text-slate-600 font-semibold">
          ...
          <label className="flex items-center gap-2 text-[11px]">
            <input
              ...
              onChange={(e) => onAllowOverride(e.target.checked)}
        ...
        <div className="grid grid-cols-2 gap-2">
          <ActionButton action="secondary" size="sm" onClick={onSkipHalftime} disabled={state.timerLocked}>
            <CornerUpRight className="w-4 h-4" />
            Salta intervallo
          </ActionButton>
          <ActionButton action="secondary" size="sm" onClick={() => onRequireExtraTime(!state.requireExtraTime)}>
      ...
      <div className="space-y-2">
        ...
        {state.matchStatus === 'suspended' ? (
          <ActionButton action="success" size="sm" onClick={onResume} className="w-full">
          ...
          <div className="space-y-2">
            ...
            <ActionButton
              action="warning"
              size="sm"
              onClick={() => suspensionReason.trim() && onSuspend(suspensionReason.trim())}
              className="w-full"
            >
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-slate-200 pt-3">
          {dangerousButton('Termina gara', onTerminateMatch, confirmTerminate, setConfirmTerminate, ShieldOff, 'bg-red-50 text-red-700 border-red-200')}
          {dangerousButton('Reset', onReset, confirmReset, setConfirmReset, RefreshCw, 'bg-slate-100 text-slate-700 border-slate-200')}
        </div>
